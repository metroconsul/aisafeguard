...
import { ShieldCheck, FileText, HardHat, AlertTriangle, ArrowRightLeft, Loader2, LogOut, Download, Eye } from "lucide-react";
...
interface Entrega {
  ...
  epi: { nome_equipamento: string; numero_ca: string } | null;
}
    ...
    <div className="min-h-screen bg-muted">
      ...
      <div className="p-4 space-y-4 max-w-lg mx-auto">
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-base flex items-center gap-2">
              <FileText className="h-5 w-5 text-primary" /> Meus Holerites
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-2">
            ...
            {holerites.map((d) => (
              <div key={d.id} className="flex items-center justify-between rounded-lg border border-border p-3">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">{d.created_at ? format(new Date(d.created_at), "dd/MM/yyyy") : ""}</p>
                </div>
                {d.file_url && (
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-base flex items-center gap-2">
              <HardHat className="h-5 w-5 text-primary" /> Meus EPIs
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-2">
          ...
          <div className="space-y-4">
            <div className="space-y-1.5">
              ...
              <Select value={trocaEpi} onValueChange={setTrocaEpi}>
                ...
                <SelectContent>
                  {epis.map((e) => (
                    <SelectItem key={e.id} value={e.id}>{e.epi?.nome_equipamento || "EPI"}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
